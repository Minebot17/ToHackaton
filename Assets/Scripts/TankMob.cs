...
public class TankMob : MonoBehaviour {
    public float speed = 1;
    public int damage = 5;
    public float radiusVision = 200;
    ...
    public void FixedUpdate() {
        int state = 0;
        RaycastHit2D ray = Physics2D.Raycast(transform.position + new Vector3(-80, 0, 0), Vector2.left, radiusVision);
        if (ray.collider != null && ray.collider.gameObject.layer == 9)
            state = 1;
        if (state == 0) {
            ray = Physics2D.Raycast(transform.position + new Vector3(80, 0, 0), Vector2.right, radiusVision);
            if (ray.collider != null && ray.collider.gameObject.layer == 9)
                state = 2;
        }
    ...
    public void DamagePlayer() {
        ...
        Physics2D.OverlapCircle(transform.position + new Vector3((transform.localScale.x < 0 ? -1 : 1) * 64, 0, 0),32, new ContactFilter2D(), colliders);
